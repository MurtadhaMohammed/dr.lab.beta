 ...
 * MetaJson editor modal (panel & composite only)
 * - Layout auto by type
 * - Builder only (no raw json tab)
 */
export const MetaJsonModal = ({
  open,
  type, // "panel" | "composite"
  name,
  initialMetaJson, // string | object
  onCancel,
  onSubmit,
}) => {
  const [panelItems, setPanelItems] = useState([]);
  ...
  }, [open, type, initialMetaJson]); // كافي

  const builderJson = useMemo(() => {
    const base = { print: { layout: isPanel ? "table" : "sections" } };
    if (isPanel) {
      return {
        ...base,
        items: panelItems.map(cleanAndNull),
      };
    }
    if (isComposite) {
      return {
        ...
        sections: sections.map((sec) => ({
          ...cleanAndNull(sec),
          fields: (sec.fields || []).map(cleanAndNull),
        })),
      };
    }
    return base;
  ...
  const handleOk = () => {
    try {
      ...
      // تحقق بسيط من الأكواد المكررة
      if (isPanel) {
        const dups = findDuplicates(panelItems.map((i) => (i.code || "").trim()).filter(Boolean));
        if (dups.length) {
          message.error(`Duplicated codes in panel: ${dups.join(", ")}`);
          return;
        }
      } else {
        const allFieldCodes = (sections || []).flatMap((s) =>
          (s.fields || []).map((f) => (f.code || "").trim()).filter(Boolean)
        );
        const dups = findDuplicates(allFieldCodes);
        if (dups.length) {
          message.error(`Duplicated field codes: ${dups.join(", ")}`);
          return;
        }
      }

      // console.log(builderJson)
      onSubmit?.(builderJson); // نرسل Object
    } catch {
      message.error("Error building JSON");
    }
  };
...
function PanelBuilder({ items, setItems }) {
  ...
  const move = (idx, dir) => {
    ...
    copy[target] = tmp;
    // أعد ترتيب order
    setItems(copy.map((r, i) => ({ ...r, order: i + 1 })));
  };
          ...
          .map((r, idx) => (
            ...
            >
              <Space direction="vertical" style={{ width: "100%" }}>
                ...
                {/* Ref editor (Text / JSON ranges) */}
                <RefCell value={r.ref} onChange={(val) => updateRow(idx, "ref", val)} />
              </Space>
            </div>
          ))
      )}
...
function CompositeBuilder({ sections, setSections }) {
    ...
    setSections(
      sections.map((s, i) =>
        i === sIdx
          ? {
              ...s,
              fields: (s.fields || []).map((f, j) => (j === fIdx ? { ...f, [key]: val } : f)),
            }
          : s
      )
    );
  ...
  //   if (!sec || target < 0 || target >= (sec.fields || []).length) return;
  //   const copy = (sec.fields || []).slice();
  //   const tmp = copy[fIdx];
  //   copy[fIdx] = copy[target];
  //   copy[target] = tmp;
  //   const updated = copy.map((f, i) => ({ ...f, order: i + 1 }));
  //   updateSection(sIdx, "fields", updated);
  // };
    ...
    <div>
      {sections.length === 0 ? (
        ...
        sections
          ...
          .map((sec, sIdx) => (
            <div
              key={`${sec.code || "sec"}-${sIdx}`}
              style={{
                padding: 12,
                border: "1px solid #eee",
                borderRadius: 8,
                marginBottom: 12,
                background: "linear-gradient(135deg, rgba(163,67,201,0.05), rgba(67,170,201,0.05))",
              }}
            >
              <div className="bg-[#f6f6f6] p-2 rounded-[8px]">
                ...
                <Space wrap>
                  ...
                  <Input
                    ...
                    placeholder="Name EN"
                    value={sec.name_en}
                    onChange={(e) => updateSection(sIdx, "name_en", e.target.value)}
                  />
                  <Input
                    style={{ width: 200 }}
                    placeholder="Name AR"
                    value={sec.name_ar}
                    onChange={(e) => updateSection(sIdx, "name_ar", e.target.value)}
                  />
                  <InputNumber
                    style={{ width: 90 }}
                    placeholder="Order"
              ...
              <div className="border-l-4 border-l-[#eee] pl-[16px]">
                {(sec.fields || []).length === 0 ? (
                  ...
                  (sec.fields || [])
                    ...
                    // .sort((a, b) => (a.order || 0) - (b.order || 0))
                    .map((f, fIdx) => (
                      <div key={`${f.code || "fld"}-${fIdx}`} style={{ marginBottom: 10 }} className="border-b border-b-1-[#eee] pb-2">
                        <Space wrap>
                          ...
                          <Input
                            ...
                            placeholder="Code"
                            value={f.code}
                            onChange={(e) => updateField(sIdx, fIdx, "code", e.target.value)}
                          />
                          <Input
                            size="small"
                            style={{ width: 170 }}
                            placeholder="Label EN"
                            value={f.label_en}
                            onChange={(e) => updateField(sIdx, fIdx, "label_en", e.target.value)}
                          />
                          <Input
                            size="small"
                            style={{ width: 170 }}
                            placeholder="Label AR"
                            value={f.label_ar}
                            onChange={(e) => updateField(sIdx, fIdx, "label_ar", e.target.value)}
                          />
                          ...
                          {/* <Space.Compact>
                            <Button size="small" onClick={() => moveField(sIdx, fIdx, -1)}>
                              ↑
                            </Button>
                            <Button size="small" onClick={() => moveField(sIdx, fIdx, +1)}>
                              ↓
                            ...
                            {/* محرّر الخيارات يظهر إذا النوع choice/presence */}
                        {(f.type === "choice" || f.type === "presence") && (
                          <div className="mx-5" style={{ marginTop: 6 }} >
                            <Text type="secondary" style={{ display: "block", marginBottom: 4 }}>
                              Choices (type to add, press Enter):
                            </Text>
                            <Select
                              mode="tags"
                              allowClear
                              ...
                              value={Array.isArray(f.choices) ? f.choices : []}
                              onChange={(vals) => updateField(sIdx, fIdx, "choices", vals)}
                            />
                          </div>
                        )}

                          <Button danger type="link" onClick={() => removeField(sIdx, fIdx)}>
                            Remove Field
                          </Button>
                        </Space>
                ...
                <Button type="dashed" onClick={() => addField(sIdx)} style={{ marginTop: 4 }}>
                  + Add Field
                </Button>
              </div>
            </div>
...
function RefCell({ value, onChange }) {
  ...
  const updateRange = (i, key, val) =>
    setRangesAndSync(ranges.map((r, idx) => (idx === i ? { ...r, [key]: val } : r)));

  const removeRange = (i) => setRangesAndSync(ranges.filter((_, idx) => idx !== i));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <Select
          ...
          onChange={(v) => {
            if (v === "json") {
              setJsonMode(true);
              if (!tryParseJsonArray(value)) {
                const init = [{ sex: null, low: null, high: null, unit: "", note: "" }];
                setRanges(init);
                onChange(JSON.stringify(init));
              }
            } else {
      ...
      {!jsonMode ? (
        ...
        <div style={{ border: "1px dashed #ddd", borderRadius: 8, padding: 8 }}>
          {ranges.length === 0 ? (
            <EmptyNote text="No ranges. Add one." />
          ) : (
            ranges.map((r, i) => (
...
function nextOrder(arr) {
  if (!Array.isArray(arr) || arr.length === 0) return 1;
  const max = Math.max(...arr.map((x) => (typeof x?.order === "number" ? x.order : 0)));
  return Number.isFinite(max) ? max + 1 : 1;
}

function cleanAndNull(obj) {
  const out = { ...obj };
  Object.keys(out).forEach((k) => {
...
function EmptyNote({ text = "Nothing to show." }) {
  return (
    <div
      style={{
        padding: 12,
        background: "linear-gradient(135deg, rgba(163,67,201,0.06), rgba(67,170,201,0.06))",
        border: "1px dashed #ddd",
        borderRadius: 8,
        color: "#666",
        fontSize: 13,
